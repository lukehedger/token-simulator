...
export const init = ({ debug = false, totalAgents = 10 } = {}) => {
  // Initialise `Simulation`
  const simulation = Simulation({ debug: debug });
  ...
  simulation.processTransaction = ({ amount, from, to }) => {
  ...
  simulation.objective = () => {
    Object.values(simulation.agents)
      .filter(agent => agent.balance >= 1)
      .map(agent => {
        const recipient = Object.values(simulation.agents).filter(
          ({ id }) => id !== agent.id
        )[
          Math.floor(
            Math.random() * (Object.keys(simulation.agents).length - 1 - 0) + 0
          )
        ];
